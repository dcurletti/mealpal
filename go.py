...
class MealPal(object):
    ...
    def get_city(self, city_name):
        ...
        if not self.cities:
            self.get_cities()
        print(self.cities, city_name)
        return next(filter(lambda x: x['name'] == city_name, self.cities))
    ...
    def get_schedule_by_restaurant_name(
        ...
        return next(filter(lambda x: x['restaurant']['name'] == restaurant_name,
                      self.schedules))
    ...
    def get_schedule_by_meal_name(
        ...
        if not self.schedules:
            self.get_schedules(city_name, city_id)
        return next(filter(lambda x: x['meal']['name'] == meal_name,
                      self.schedules))
    ...
    while True:
        try:
            status_code = mp.reserve_meal(
                '12:15pm-12:30pm',
                restaurant_name='Tender Greens - King St.',
